...
<html>
 ...
 mysql_query($query) or die(mysql_error());
		...
		$query="select DISTINCT YEAR(doj) AS year from employee_details where dept='".$dept."'";
		$result=mysql_query($query);
		while($row=mysql_fetch_assoc($result))
		{
		$year=$row['year'];
		$query1="select count(*) AS no from employee_details where doj LIKE('".$year."-%-%') AND dept='".$dept."'";
		...
		$chart->setTitle("Yearly employee turnover for '".$dept."'");
		$chart->render("images/to.png");
	
	 echo "<img alt='Line graph'  src='images/to.png'/>";
		?>
		</center>
		<a href='turnover_select.php' style="text-decoration:none;padding-left:900px;color:red">Back</a>
        </div>
		</body>
	</html>