...
<body>
 <div style="position:absolute;top:100px;left:200px;height:300px;width:500px;padding-left:150px;">
<?php
    //include the library
 ...
 $query='select sum(mostly) as Mostly from student_feedback where teacher="'.$id.'"';
...
$query='select sum(sometimes) as Sometimes from student_feedback where teacher="'.$id.'"';
$result=mysql_query($query) or die(mysql_error());
while($row=mysql_fetch_assoc($result))
 {
...
$query='select sum(hardly) as Hardly from student_feedback where teacher="'.$id.'"';
 $result=mysql_query($query) or die(mysql_error());
 while($row=mysql_fetch_assoc($result))
 {
 $h=$row['Hardly'];
...
?>
 ...
 <a href='select.php' style='text-decoration:none;color:black;float:right;'>Back</a>
 </div>
</body>
</html>